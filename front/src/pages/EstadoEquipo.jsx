...
class App extends React.Component {
  ...
  listar = async () => {
    try {
      const resp = await fetch("http://localhost:3030/estado-equipo", { method: "GET" })
  ...
  editar = async (dato) => {
    ...
    try {
      const resp = await fetch(`http://localhost:3030/estado-equipo/${dato._id}`, {
  ...
  eliminar = async (id, index) => {
    var opcion = window.confirm("Estás Seguro que deseas Eliminar Estado Equipo");
    if (opcion == true) {
      try {
        const resp = await fetch(`http://localhost:3030/estado-equipo/${id}`, {
          method: "DELETE",
        })
        const data = await resp.text()
  ...
  insertar = async () => {
    try {
      const resp = await fetch("http://localhost:3030/estado-equipo", {
    ...
    return (
      <>
        <Container >
          ...
          <Button color="success" onClick={() => this.mostrarModalInsertar()}>Nuevo  Estado Equipo</Button>
        ...
        <Modal isOpen={this.state.modalActualizar}>
          <ModalHeader>
            <div><h3>Editar Estado Equipo</h3></div>
          </ModalHeader>
