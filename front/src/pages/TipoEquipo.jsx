...
class App extends React.Component {
  ...
  listar = async () => {
    try {
      const resp = await fetch("http://localhost:3030/tipo-equipo", { method: "GET" })
  ...
  editar = async (dato) => {
    ...
    try {
      const resp = await fetch("http://localhost:3030/tipo-equipo/"+dato._id, {
  ...
  eliminar = async (id, index) => {
    var opcion = window.confirm("Estás Seguro que deseas Eliminar el equipo");
    if (opcion == true) {
      try {
        const resp = await fetch(`http://localhost:3030/tipo-equipo/${id}`, {
          method: "DELETE",
        })
        const data = await resp.text()
  ...
  insertar = async () => {
    try {
      const resp = await fetch("http://localhost:3030/tipo-equipo", {
    ...
    return (
      <>
        <Container >
          ...
          <Button color="success" onClick={() => this.mostrarModalInsertar()}>Crear Nuevo Equipo</Button>
        ...
        <Modal isOpen={this.state.modalInsertar}>
          <ModalHeader>
            <div><h3>Registrar Equipo </h3></div>
          </ModalHeader>
